        ...
        for key , value in dict_design.items():
            ...
            for index, row in df.iterrows():
                ...
                for run in range(1, value[1]+ 1):
                    ...
                    noise = np.random.normal(0, sigma, value[2])
                    data = noise + deterministic
            ...
            outdir = "normal" + str(alpha) + "_"+ str(tot_rep)
            if not os.path.exists(outdir):
                os.mkdir(outdir)

            filename = "normal{}.csv".format(key)
