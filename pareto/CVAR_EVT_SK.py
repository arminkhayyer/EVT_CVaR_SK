...
files = ["pareto_evt1.csv","pareto_evt2.csv","pareto_evt3.csv","pareto_evt4.csv","pareto_evt5.csv","pareto_evt6.csv","pareto_evt7.csv","pareto_evt8.csv","pareto_evt9.csv"]

# files = ["pareto_evt1.csv", "pareto_evt2.csv", "pareto_evt3.csv", "pareto_evt4.csv"]
# files = ["pareto_evt3.csv"]
test_files = sorted(os.listdir("pareto_test"))
...
def True_cvar(x1, x2, alpha):
    ...
    true_val = mu + ((2 + sigma) * 2) / math.sqrt(alpha)
    return true_val

for i in files:
    num_identifier = int(i[-5])
    num_rep = dict_design[num_identifier][1]
    filename = "pareto/" + i
    df = pd.read_csv(filename)
    df = df.drop(["Unnamed: 0"], 1)

    filename_test = "pareto_test99/" + "pareto_evt1.csv"
    df_test = pd.read_csv(filename_test)
    df_test = df_test.drop(["Unnamed: 0"], 1)
    df_test = df_test.sort_values(by=["POT_conditional_va_risk"])
