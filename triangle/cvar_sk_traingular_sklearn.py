...
files = ["triangular{}.csv".format(i) for i in range(1, 16)]
    ...
    for tot_rep in range(10):
        for var in vars:
            ...
            for i in files:
                try:
                    print(i)
                    num = i.strip("triangular")
                    num = num.strip(".csv")
                    ind = int(num)
                    num_rep = dict_design[ind][1]
                    num_identifier = ind
                    print(tot_rep,i, ind, var)
                    filename ="triangular" +str(alpha) + "_"+ str(tot_rep) + "/"+ i
                    df = pd.read_csv(filename)
                    df = df.drop(["Unnamed: 0"], 1)

                    filename_test = "triangular_test"+ str(alpha) + "/triangular.csv"
                    ...
                    kernel = C(thou2, (1e-3, 1e3)) * RBF([.10, .1], (1e-3, 1e3))
                    ...
                    result_dir = "triangular{}_{}/".format(alpha,tot_rep)+"res{}.csv".format(vars.index(var))
