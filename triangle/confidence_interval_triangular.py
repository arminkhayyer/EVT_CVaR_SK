...
for alpha in alphas:
    dirs = []
    for rep in range(10):
        dirs.append("triangular{}_{}".format(alpha, rep))
    ...
    df.to_csv("all_res_triangular{}.csv".format(alpha))
    ...
    for i in range(len(df_sample)):
        ...
        temp = [round(i, 2) for i in data if i<=25]
        cfd = st.t.interval(alpha=0.95, df=len(temp) - 1, loc=np.mean(temp), scale=st.sem(temp))
        cdf_round = [round(i, 2) for i in cfd]
        confidence.append(str(cdf_round))
        Mean.append(np.median(temp))
    ...
    plt.tight_layout()
    plt.savefig("triangular{}.pdf".format(alpha))
    plt.show()


    print(df_final)
    df_final.to_csv("final_result_triangular{}.csv".format(alpha))
    empty_list = []
